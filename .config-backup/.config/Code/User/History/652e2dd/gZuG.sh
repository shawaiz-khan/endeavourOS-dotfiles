...
if [ "$BATTERY_LEVEL" -ge 100 ] && [ "$BATTERY_STATE" = "fully-charged" ]; then
    ...
    notify-send "Battery Critical" "Battery is critical (${BATTERY_LEVEL}%). Plug in now!" -u critical -i "battery-alert"
elif [ "$BATTERY_LEVEL" -le $WARNING_LEVEL ] && [ "$BATTERY_STATE" = "discharging" ]; then
    notify-send "Low Battery" "Battery is low (${BATTERY_LEVEL}%)." -u normal -i "battery-alert"
fi